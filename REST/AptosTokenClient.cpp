...
std::vector<uint8_t> Property::SerializeValue() {
    Serialization ser;
...
//        types.push_back(prop.PropertyType);
...
//    PropertyMapResourceData propMapResData = dynamic_cast<PropertyMapResourceData&>(resource);
//    std::vector<Property> properties;
//    std::vector<PropertyResource> props = propMapResData.Inner.Data;

//    for (const PropertyResource& prop : props) {
//        properties.push_back(Property::Parse(
//            BString(prop.Key),
//            std::stoi(prop.Value.Type),
//            Bytes(prop.Value.Value.ByteArrayFromHexString())
//            ));
//    }
...
std::string PropertyMap::GetStructTag() {
    return StructTag;
}
