...
Authenticator::Authenticator(const std::shared_ptr<IAuthenticator>& authenticator) :
    Variant(Authenticator::ED25519), m_authenticator(authenticator)
{
...
std::shared_ptr<ISerializable> Authenticator::Deserialize(Deserialization &deserializer) {
    ...
    std::shared_ptr<ISerializable> authenticator = nullptr;
...
bool Authenticator::Equals(const Authenticator &other) const {
    ...
    if (vr) {
        if (Variant == Authenticator::ED25519) {
            vl = std::dynamic_pointer_cast<Ed25519Authenticator>(m_authenticator)->Equals(
                *(std::dynamic_pointer_cast<Ed25519Authenticator>(other.m_authenticator)));
        } else if (Variant == Authenticator::MULTI_ED25519) {
            ...
            vl = std::dynamic_pointer_cast<MultiAgentAuthenticator>(m_authenticator)->Equals(
                *(std::dynamic_pointer_cast<MultiAgentAuthenticator>(other.m_authenticator)));
        } else {
            return false;
        }
        return vr && vl;
...
std::shared_ptr<ISerializable> Ed25519Authenticator::Deserialize(Deserialization &deserializer) {
    return nullptr;
//    auto key = PublicKey::Deserialize(deserializer);
//    auto signature = Signature::Deserialize(deserializer);
...
std::shared_ptr<ISerializable> MultiAgentAuthenticator::Deserialize(Deserialization &deserializer) {
    ...
    std::vector<std::shared_ptr<ISerializable>> secondaryAddressesSeq = deserializer.DeserializeSequence(
        [](Deserialization& d) -> std::shared_ptr<ISerializable> { return AccountAddress::Deserialize(d); });
    std::vector<std::shared_ptr<ISerializable>> authenticatorsSeq =  deserializer.DeserializeSequence(
        [](Deserialization& d) -> std::shared_ptr<ISerializable> { return Authenticator::Deserialize(d); });
...
bool MultiAgentAuthenticator::Equals(const MultiAgentAuthenticator &other) const {
    return (sender.Equals(other.sender) && secondarySigners == other.secondarySigners);
}
