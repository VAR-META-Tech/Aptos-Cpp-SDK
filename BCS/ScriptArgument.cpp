...
ScriptArgument::ScriptArgument(TypeTag variant, std::shared_ptr<ISerializableTag> value) {
    if (variant < TypeTag::U8 || variant > TypeTag::BOOL) {
...
void ScriptArgument::Serialize(Serialization &serializer) const {
    ...
    else if (this->variant == TypeTag::ACCOUNT_ADDRESS) {
...
std::shared_ptr<ISerializable> ScriptArgument::Deserialize(Deserialization &deserializer) {
    TypeTag variant = static_cast<TypeTag>(deserializer.DeserializeU8());
    std::shared_ptr<ISerializableTag> value;

    if (variant == TypeTag::U8) {
